...
app.get("/DownloadFile", async function (request, result) {
    ...
    if (link != null) {
        fileSystem.readFile(link.file.filePath, function (error, data) {
            ...
            result.setHeader('Content-disposition', 'attachment; filename=' + link.file.name);
        ...
        app.post("/UploadFile", async function (request, result) {
            if (request.session.user) {
                ...
                if (request.files.file.size > 0) {
                    ...
                    //     fileSystem.mkdirSync("./uploads/" + user.email);
                    // }
                    if (!fileSystem.existsSync("public/uploads/")) {
                        try {
                          fileSystem.mkdirSync("public/uploads/", { recursive: true });
                        } catch (err) {
                          if (err.code !== 'EXIST') {
